...
fn get_parent_child_from_digraph_line(line: &str) -> Option<(Purl, Purl)> {
    ...
    if let Some(left) = left.strip_prefix("[INFO]") {
        ...
        return match (get_purl_from_digraph_string(left), get_purl_from_digraph_string(right)) {
...
fn calculate_build_dependencies(tree: TreeParse) -> anyhow::Result<BuildDependencies> {
    ...
    let direct: Vec<&Purl> = tree.parent_and_child.iter()
...
fn find_all_dependencies_of(tree: &TreeParse, purl: &Purl, dependencies: &mut Vec<Purl>) {
    ...
    // Find the children
    let children: Vec<Purl> = tree.parent_and_child.iter()
        .filter(| value| value.0.eq(purl))
        .map(|entity| entity.1.clone())
        .collect();

    children.iter().for_each(|purl| {
        find_all_dependencies_of(&tree, &purl, dependencies)
    });
}
...
mod test {
    ...
    fn parse_parent_child_into_build_dependencies() {
        let tree = TreeParse {
            ...
            application_purl: get_purl_from_digraph_string("com.javatpoint.application1:my-application1:jar:1.0"),
            parent_and_child: vec![
                (
                    get_purl_from_digraph_string("com.javatpoint.application1:my-application1:jar:1.0").unwrap(),
                    get_purl_from_digraph_string("org.opensaml:opensaml:jar:2.4.1:compile").unwrap()
                ),
                (
                    get_purl_from_digraph_string("com.javatpoint.application1:my-application1:jar:1.0").unwrap(),
                    get_purl_from_digraph_string("org.apache.struts:struts-core:jar:1.3.10:compile").unwrap()
                ),
                (
                    get_purl_from_digraph_string("org.opensaml:opensaml:jar:2.4.1:compile").unwrap(),
    ...
    fn can_get_purl_from_digrph_string() {
        let result = get_purl_from_digraph_string("com.javatpoint.application1:my-application1:jar:1.0");

        assert_eq!(result.unwrap(), Purl {
            package: "maven".to_string(),
    ...
    fn should_move_to_start_state_if_start_found() {
        ...
        look_for_start_state(&mut tree, "[INFO] digraph \"com.javatpoint.application1:my-application1:jar:1.0\" {");
    ...
    #[test]
    fn parse_file() {
        let data = include_str!("../../../resources/maven/tree_response.txt");
